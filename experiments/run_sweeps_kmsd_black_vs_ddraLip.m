function SUMMARY = run_sweeps_kmsd_black_vs_ddraLip(cfg, grid)
% Sweep over n_m for k-MSD using:
      ...
      for n_s = axes.n_s
        for n_k = axes.n_k
          for ip = 1:numel(axes.pe)
            ...
            dyn    = getfielddef(shared,'dyn',"kMSD");
            ...
            % p = sys.n_p if present (ARX); else 0 for state-space k-MSD
            p_hist = getfielddef(sys,'n_p',0);
            options.approx = struct('p', p_hist);
            ...
            % Conservatism proxy (BLACK) — intervals on learned model
            t_inf = tic;
            ...
            % ======== DDRA-Lipschitz (Alg. 6) ========
            % Build data D from training suite (use detected history p_hist)
            D  = build_D_from_suite(params_true.testSuite_train, p_hist);
            nx = size(D.Xminus,1);
            ...
            for mval = 1:length(params_true.testSuite_val)
                ...
                % Build input stack Uk with p_hist lags (k, k-1, ..., k-p_hist)
                Uk = cell(1, T);
                for k = 1:T
                    u_stack = [];
                    for lag = 0:p_hist
                        idx = max(1, k-lag);
                        u_stack = [u_stack; Uv(:, idx)]; 
                    end
                ...
                % Ensure measurement dimensions line up: TS.y is (T × ny × s) in CORA testCase
                % We need (ny × T × s)
                Y_perm = permute(TS.y, [2 1 3]);

                % ddra_reach_lipschitz often returns T+1 sets (incl. initial set); align
                Xuse = Xsets;
